-- offsetX/Yの変化の代わりにmovementX/Yを用いる別解
-- これも非常にスムーズだがSafari等ではサポートされない
module Main_alt2 exposing (main)
...
update msg model =
  case msg of
      ...
      Move x1 y1 ->
          if model.dragging
          then { model | x = model.x + x1, y = model.y + y1}
          else model
...
view model =
  ...
  in
    div [ style "position" "absolute"
        ...
        , on "mousemove" <| map2 Move (field "movementX" int) (field "movementY" int)
        ]
        [ text <| "(x=" ++ x ++ ",y=" ++ y ++ ")"
        ]
